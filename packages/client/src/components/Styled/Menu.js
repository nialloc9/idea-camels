...
export const Menu = styled(({ fontSize, color, anchorColor, backgroundColor, mobileDisplay, display, height, mobileHeight, ...rest }) =>
    <SemanticMenu {...rest} />
)`
    ${({ fontSize = false }) =>  fontSize && `font-size: ${fontSize};`}
    ${({ backgroundColor = false }) =>  backgroundColor && `background-color: ${backgroundColor} !important;`}
    ${({ fontFamily = false }) =>  fontFamily && `font-family: ${fontFamily} !important;`}
    ...
    a {
        ${({ anchorColor = false }) => anchorColor && `color: ${anchorColor} !important;`}
    }
...
export const Item = styled(({ padding, margin, color, cursor, mobileDisplay, backgroundColor, hoverBackgroundColor, ...rest }) =>
    <SemanticItem {...rest}  />
)`
    ...
    &:hover {
        ${({ hoverBackgroundColor = false }) => hoverBackgroundColor && `background-color: ${hoverBackgroundColor};`}
    }
`;
