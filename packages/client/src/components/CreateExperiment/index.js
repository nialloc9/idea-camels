...
class CreateExperiment extends Component {
    ...
    constructor(props) {
        ...
        this.state = { domains: this.mapDomains(domains.map(({ name }) => name)), domainError: undefined, domainPrice: undefined }
    }
    ...
    get themeOptions() {
        const { templates = [], values: { templateRef } } = this.props;
    ...
    includesDomain = () => {
        const { domains, values: { domain } } = this.props 
        console.log(domains)
    ...
    handleAddDomain = (e, { value }) => {
        ...
        const newState = { domainError, domains: domainError ? domains : [this.createDomainValue(value), ...domains] }
    ...
    renderSuggestDomains = () => {
        ...
        if(suggestedDomains.length > 0) {
            return (
                ...
                <List>
                    {suggestedDomains.map(o => <ListItem key={o}>{o}</ListItem>)}
                </List>
                </Message>
            )
        }
    ...
    render() {
        const { submitting, pristine, isFetchTemplatesLoading, submitError, values: { domain }, newExperiment: { templateRef, themeRef }, onSubmit } = this.props;
        ...
        return (
            <Form error={submitError} onSubmit={onSubmit}>
                            <Grid container centered stackable>
                                <GridRow centered columns={2}>
                                    <GridColumn>
                                            <FormDropdown 
                                                ...
                                                fluid
                                                allowAdditions
                                                label={domainPrice && !this.includesDomain() ? <div style={{ color: 'red' }}>{`Domain (Warning: You will be charged $${domainPrice} for purchase of new domain ${domain})`}</div> : 'Domain'}
                                                name="domain"
                                ...
                                <GridRow>
                                    <GridColumn><Button disabled={submitting || pristine} isLoading={submitting}>Create</Button></GridColumn>
                                </GridRow>
                            </Grid>
                        </Form>
            )
...
export default connect(
    ({ experiment: { newExperiment, templates }, domain: { data: domains, suggestedDomains, prices } }) => ({ newExperiment, templates, domains, suggestedDomains, domainPrices: prices }),
