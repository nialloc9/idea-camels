...
export class ControledFormWizard extends Component {
  ...
  onSelectPreviousForm = () =>
    this.setState(({ index }) => ({ index: index === 0 ? 0 : index - 1 }));

  onSelectNextForm = () =>
    this.setState(({ index }) => ({
      index: index === this.props.maxIndex ? index : index + 1,
    }));
  ...
  render() {
    ...
    return (
      <FormWizard
        ...
        forms={forms}
        {...rest}
        onSelectPreviousForm={this.onSelectPreviousForm}
        onSelectNextForm={this.onSelectNextForm}
      />
    );
  }
}
