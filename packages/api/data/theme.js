  ...
  new Promise(async (resolve, reject) => {
    try {
      ...
      const results = await query(
        ...
        undefined,
        caller,
        "GET_THEME_BY_THEME_REF"
      );

      resolve(
        handleSuccess(`DATA - GET_THEME_BY_THEME_REF - FROM ${caller}`, results)
      );
    } catch (error) {
      reject(error);
    }
...
const onCreate = ({ data, caller }) =>
  new Promise(async (resolve, reject) => {
    try {
      ...
      resolve(
        handleSuccess(`DATA - CREATE_THEME - FROM ${caller}`, {
          ...mappedData,
 ...
 */
const onUpdate = ({
  data: { themeRef, lastUpdatedBy, data: updateData },
  caller,
}) =>
  new Promise(async (resolve, reject) => {
    try {
      const updateQuery = `UPDATE themes SET ? WHERE theme_ref='${themeRef}'`;
      ...
      resolve(
        handleSuccess(`DATA - UPDATE_THEME - FROM ${caller}`, {
          ...data,
