...
resource "aws_lambda_permission" "lambda_api" {
  ...
  source_arn = "${aws_api_gateway_rest_api.lambda_api.execution_arn}/*/POST/*"
}
...
resource "aws_iam_policy" "api_permissions" {
  name        = "${var.environment}_api_permissions"
  path        = "/"
  description = "IAM policy for api permissions for ${var.environment} lambda"
